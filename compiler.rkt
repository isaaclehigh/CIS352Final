...
(define (ifarith->ifarith-tiny e)
  (match e
    ...
    ;; 0-binding case
    [`(let* () ,e) `(let () ,(ifarith->ifarith-tiny e))]
    ;; 1+-binding case
    [`(let* ([,(? symbol? x0) ,e0]) ,e-body)
     `(let ([,(ifarith->ifarith-tiny x0) ,(ifarith->ifarith-tiny e0)]) ,(ifarith->ifarith-tiny e-body)) ]
    [`(let* ([,(? symbol? x0) ,e0] ,rest-binding-pairs ...) ,e-body)
     `(let ([,(ifarith->ifarith-tiny x0) ,(ifarith->ifarith-tiny e0)])
